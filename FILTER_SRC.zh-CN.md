...
runtime 暴露固定的 Lua 库表面。脚本不应依赖此列表之外的全局值。
...
`math` 库包含 Lua 风格的 `math.random(...)` 和 `math.randomseed(seed)`。除非脚本显式调用 `math.randomseed(seed)`，否则随机输出不保证可复现。

`os`、`io`、`package` 和 `debug` 库不属于 runtime contract。
...
`time` 与渲染时间线分离。frame time、delta time 和 frame index 是 runtime 执行概念，应属于 `ctx` API，而不是 `time`。
...
`time.parts(out, timestamp [, options])` 把 timestamp 拆成日历字段。

`timestamp` 是单位为秒的 Unix time。它可以包含小数部分。

`out` 必须是 Lua table。runtime 会把字段写入 `out` 并返回 `out`，因此作者可以跨帧复用同一个 table。

`options` 是可选 Lua table。支持字段：

- `utc`：为 `true` 时使用 UTC。否则使用 runtime 的本地时区。
...
- `sec`：`0` 到 `59`
- `millis`：`0` 到 `999`
- `wday`：`1` 到 `7`，Sunday 为 `1`
- `yday`：`1` 到 `366`，January 1 为 `1`
...
`time.fromDate(date [, options])` 把日期字段转换成单位为秒的 Unix timestamp。

`date` 是 Lua table。`time.fromDate` 只读取这些字段：
...
输入日期字段会先按日历规则归一化，再转换为 timestamp。`wday` 和 `yday` 等派生字段不会被 `time.fromDate` 读取。

`options` 是可选 Lua table。支持字段：

- `utc`：为 `true` 时把输入解释为 UTC。否则按 runtime 的本地时区解释。
...
`time.parts` 和 `time.fromDate` 使用彼此独立的 table 形状。部分字段名相同，只是因为它们描述相同的日历概念；`time.parts` 的输出不是 `time.fromDate` 要求的输入结构。
